    ...
    <div>
        ...
        <div class="container">
            <div class="row justify-content-between">
                <div class="col-4">
                    <RouterLink class="btn btn-primary m-2 float-end" to="/add-empleados"><i class="bi bi-person-plus"></i> Agregar
                    empleado</RouterLink>
                </div>
                <div class="col-4">
                   <div class="input-group ">
                        <input v-model="search" type="text" class="form-control m-2 float-end" placeholder="Buscar...">
                        <!--input v-model="search" type="text" class="form-control m-2 float-end" placeholder="Buscar..." v-on:keyup="searchData">                        
                        <button class="btn btn-success m-2 float-end " v-on:click="searchData" ><i class="bi bi-search"></i> Buscar empleado</button-->
                    </div>
                </div>
            </div>
        </div>        
        ...
        <table class="table table-striped">
            ...
            <tbody>
                <tr v-for="(emp, index) in this.empleados" :key="index">
                    ...
                    <td>{{ emp.placa }}</td>
                    <td>
                        <RouterLink rel="Editar" class="btn btn-warning" :to="`/edit-empleados/${emp.id}`"><i class="bi-pencil-square"></i>
                        </RouterLink>
                        <RouterLink rel="Eliminar" class="btn btn-danger btn-md" :to="`/delete-empleados/${emp.id}`"><i class="bi bi-trash"></i>
                        </RouterLink>
