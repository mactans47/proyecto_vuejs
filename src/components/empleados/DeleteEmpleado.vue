...
onMounted(async() => {
    axios.get(`http://localhost:3000/employees/${route.params.id}`)
    .then((response) => {
    ...
    const responset = await axios.get('http://localhost:3000/types'); // Cambia la URL si es necesario
...
const deleteEmpleado = () =>{
    axios.delete(`http://localhost:3000/employees/${route.params.id}`, updateEmpleado)
    .then(() => {
...
<template>
    <div class="container w-50">
        <div class="card">
            ...
            <div class="card-body">
                <form @submit.prevent="deleteEmpleado">
                    <div class="mb-3">
                            <label for="">Nombre</label>
                            <input type="text" v-model="updateEmpleado.name" class="form-control" id="txtName" disabled/>
                        </div>
                        
                        <div class="mb-3">
                            <label for="">Area</label>
                            <select v-model="selectedAreaId" id="user-select" class="form-control" disabled>
                        ...
                        <div class="mb-3">
                            ...
                            <select v-model="selectedTipoId" id="tipo-select" class="form-control" disabled>
                                <option v-for="(tipo, index) in tipos" :key="index" :value="tipo.id">{{tipo.name}}</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="">Color</label>
                            <input type="text" v-model="updateEmpleado.color" class="form-control" id="txtColor" disabled/>
                        </div>
                        <div class="mb-3">
                            <label for="">Placa</label>
                            <input type="text" v-model="updateEmpleado.placa" class="form-control" id="txtPlaca" disabled/>
                        </div>
                        <div class="mb-3">
